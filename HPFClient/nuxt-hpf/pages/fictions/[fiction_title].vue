<template>
  <div id="main-container" class="container pt-3" style="background-color: red; ">
    <div class="columns" style="height: 100%;">
      ...
      <div :class="['column', 'is-flex', 'is-flex-direction-column']" style="background-color: blue;">
        <div class="is-flex-grow-5">
          ...
          <NuxtChild :chapter-prop="chapter" :test-prop="'Test'" @change="(value) => (chapter = value)" />
        ...
        <div class="is-flex is-flex-direction-row is-justify-content-space-between">
          <!-- Previous -->
          <b-button v-if="previousItem != null" type="is-primary" outlined icon-left="angle-left" @click="toPrevious">
...
import { getFanfictions, getTableOfContent } from "~/api/fanfictions";
import { getChapters } from "~/api/chapters";
import { FanfictionModel, FanfictionEntityConfig, ChapterModel, TableOfContent, ChapterModelLight } from "~/models/fanfictions";
...
export default class Fanfiction extends Vue {
  ...
  private async fetch(): Promise<void> {
    ...
    try {
      ...
      if (this.fiction != null) {
        ...
        this.previousItem = (this.chapter.order === 1) ? this.tableOfContent : (this.tableOfContent?.chapters?.find((chapter: ChapterModelLight) => chapter.order === ((this.chapter?.order ?? 0) - 1)) ?? null);
        this.nextItem = this.tableOfContent?.chapters?.find((chapter: ChapterModelLight) => chapter.order === ((this.chapter?.order ?? 0) + 1)) ?? null;
      } else {
        // On verra
        this.previousItem = null;
        this.nextItem = null;
  ...
  public toPrevious(event: any): void {
    console.log(event);
  }
  ...
  public toNext(): void {
    ...
    // this.$router.push({ name: "chapitres-chapter_title", params: { id: this.nextItem?.id.valueOf(), slug: this.nextItem?.title ?? "" } });
  }
