...
class IsOwner(DjangoPermissionExtension):
    """Vérifie que l'utilisateur accédant à la ressource est le propriétaire de la ressource"""

    DEFAULT_ERROR_MESSAGE: ClassVar[str] = "L'utilisateur n'est pas le propriétaire de la ressource."
    SCHEMA_DIRECTIVE_DESCRIPTION: ClassVar[Optional[str]] = _desc(
        "Ne peut être résolu que par le propriétaire de la ressource.",
    )
    ...
    def resolve_for_user(self, resolver: Callable, user: Optional["UserType"], *, info: Info, source: Any) -> AwaitableOrValue[Any]:
        if (
            ...
            or not getattr(source, self._owner_field) == user
        ):
